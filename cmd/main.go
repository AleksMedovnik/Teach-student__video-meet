...
import (
	// Контекст сигнализирует прекращение работы
	"context"
	"fmt"
	"os"
	"time"
	"log"
	// завершить после получения kill
	"os/signal"
	// базовые примитивы синхронизации
	"sync"
	// Пакет системного вызова содержит интерфейс к низкоуровневым примитивам ОС
	// Устаревший: этот пакет заблокирован!
	"syscall"

	// микрофреймворк для обработки запросов
...
func main() {
	...
	// recover - восстановление после panic с передачей управления ErrorHandler
